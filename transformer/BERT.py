...
class BERTrainer:
    def __init__(self,
        ...
        logging.info(f"Attempt importing training dataset from {config.BIG_DATA_FILE_CLEANED}")
        ...
        self.df = self.df[self.df['confidence'] < threshold] # filter by confidence threshold
        ...
        df_train, df_test = train_test_split(self.df, test_size=test_size, shuffle=True)
        ...
        self.tokenized_train_dataset = train_dataset.map(self.tokenize_function, batched=True)
        self.tokenized_test_dataset = test_dataset.map(self.tokenize_function, batched=True)
    ...
    def sanity_check(self):
        ...
        if set(['text', 'preprocess_text', 'label', 'confidence']).issubset(self.df.columns):
            logging.info("All required columns exists")
        else:
            raise ValueError("Dataset must contain columns 'text', 'preprocess_text', 'label', 'confidence'")
    ...
    def train(self,
        ...
        logging.info(f"Starting training on train set")
        self.trainer.train()
